...
def main():
    ...
    maxlen = 128
    bert_model = "bert-base-uncased"
    bs = 16  # Further reduced batch size to prevent out-of-memory issues with larger datasets
    iters_to_accumulate = 2
    lr = 1e-5  # Slightly reduced learning rate for finer tuning
    epochs = 10  # Increased number of epochs
    set_seed(1)

    df = pd.read_csv('data/train.tsv', delimiter='\t')
    df = df.head(2000)  # Use a larger dataset if available
    dataset = CustomDataset(df, maxlen, bert_model)
    ...
    optimizer = AdamW(model.parameters(), lr=lr, weight_decay=1e-2)
    early_stopping = EarlyStopping(patience=3, verbose=True)  # Increased patience

    if torch.cuda.device_count() > 1:
        model = nn.DataParallel(model)

    num_warmup_steps = 50  # Adjusted warmup steps
    t_total = int((len(dataset) * 0.8) // iters_to_accumulate) * epochs
    lr_scheduler = get_linear_schedule_with_warmup(optimizer=optimizer, num_warmup_steps=num_warmup_steps, num_training_steps=t_total)
